...
const BACKGROUND_PATH = new URL("../background.js", import.meta.url);
...
function renderPopularDomainsBlock(domains, sourceLabels, { limit, sourceLimit }) {
  const domainLines = domains.map(({ domain }) => `  "${domain}",`).join("\n");

  return `const POPULAR_DOMAINS = new Set([
...
async function updateBackground(block) {
  const text = await readFile(BACKGROUND_PATH, "utf8");
  const pattern = /const POPULAR_(?:KR_)?DOMAINS = new Set\(\[[\s\S]*?\n\]\);/;
  if (!pattern.test(text)) {
    throw new Error("Could not find POPULAR_DOMAINS block in background.js");
  }

  const next = text.replace(pattern, block);
  await writeFile(BACKGROUND_PATH, next);
  return next !== text;
}
...
async function main() {
  ...
  const block = renderPopularDomainsBlock(
    domains,
    sources.map((source) => source.label),
    { limit, sourceLimit },
  );
  const changed = await updateBackground(block);

  console.log(`Generated ${domains.length} popular domains from ${rankedSources.length} ranked source(s).`);
  console.log(changed ? "background.js updated." : "background.js already up to date.");
}
