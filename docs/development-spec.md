...
| `clickGuardWarnApprovalStatus` | `{ url }` | `{ approved, host?, expiresAt? }` | click guard 전용 **host 단위** warn 재확인 생략 상태 조회 (url 에서 host 파생) |
| `rememberClickGuardWarnApproval` | `{ url, score? }` | `{ ok, host?, expiresAt? }` | click guard warn 확인을 세션에 host 단위로 기록. allowlist 아님 |
...
| `WHOIS_PARSE` | yesnic HTML에서 7개 WHOIS 필드(Domain Name/Registrar/Registered/Updated/Expires/Name Server/Contact) 압축 + **`Registrant`(영문) / `등록인`(한글) 을 별도 `\| Registrant: <org>` 세그먼트로 추출** — `.kr` 레지스트리는 registrant 를 노출하지만 rdap.org 는 `.kr` 미서빙(404)이라 RDAP 경로로는 못 받음. override 의 `\|` split + `^Registrant:` 필터가 인식하도록 콤마-조인 밖 세그먼트로 분리 |
...
| FR-008 | click guard는 위험 URI/data executable은 즉시 차단하고, 다운로드/copy/social 버튼은 페이지 스캔 후 진행 여부를 결정해야 한다. | danger verdict는 클릭 차단, warn은 사용자 확인 후 **같은 host 에 대해** 이번 브라우저 세션 동안 재확인 생략, safe는 원 클릭 재실행. warn 승인은 click guard 전용 `clickGuardWarnApprovals` 세션 키에 **host 단위**로 저장하며 host allowlist / safeHosts 로 승격하지 않는다. **host 단위인 이유: 가입/동의 같은 멀티스텝 플로우가 단계마다 URL(query/hash/path)을 바꾸면 exact-URL 키는 매 단계 confirm 무한 재등장(FP loop)을 유발했다. danger(>=7)는 이 승인과 무관하게 항상 차단되므로 host 단위 warn 승인은 안전.** social 버튼 트리거(`STRICT_COPY_HINT_RE`)는 ClickFix 구절("보안 확인"/"verify you are human"/"복사"/"i'm not a robot")만 매칭하고, 단독 "확인"(OK)·"인증"(본인인증)·단독 "verify" 는 정상 한국 UI 의 흔한 라벨이라 제외한다(정상 모달/가입 페이지 FP 방지). |
...
| FR-012 | OCR은 로컬 Tesseract 파일만 사용해야 한다. | CDN 로드 금지, `lib/*` 파일 진단 가능. **`createWorker` 는 `workerBlobURL:false` 로 호출 — 기본값(true)은 blob 워커가 `importScripts("chrome-extension://.../lib/worker.min.js")` 를 시도하는데 Chrome 148+ 가 blob-origin 워커의 extension URL importScripts 를 NetworkError 로 차단(OCR 전체 실패). `false` 면 extension 페이지(offscreen)와 same-origin 인 `new Worker(workerPath)` 직접 로드라 통과. OCR 실패는 offscreen `catch`→`""` + SW `.catch(()=>"")` 로 non-fatal(빈 OCR 슬라이스로 스캔 계속).** **또한 `manifest.json#content_security_policy.extension_pages` 에 `'wasm-unsafe-eval'` 필요 — Tesseract core 가 `WebAssembly.instantiate()` 를 쓰는데 기본 MV3 CSP(`script-src 'self'`)는 wasm 컴파일을 차단(CompileError)한다. `'wasm-unsafe-eval'` 은 wasm 컴파일만 허용(임의 `unsafe-eval` 아님).** **이미지는 원본 blob 을 직접 넘기지 않고 `createImageBitmap`→`OffscreenCanvas`→PNG(`convertToBlob`)로 정규화해 recognize 에 전달 — Leptonica 가 못 읽는 포맷(WebP/AVIF/SVG)·비-이미지 응답이 "Unknown format / cannot be read" 로 워커를 abort 시키던 회귀 방지. recognize 실패 시 `_ocrWorker` 를 폐기·재생성(self-heal)해 죽은 워커 재사용을 막는다.** |
...
| FR-016 | 결정론적 override는 LLM보다 우선해야 한다. | O0/O1/O1-whois/O1-whois-transitive/O1-infra-corroboration/O2/O3/O4/O7/D1/O5/O6/O8-saas/O9-first-party-install/O10-model-consistency/O11-third-party-form-provider/O12-third-party-provider-page/O13-established-kr-registrant/O14-brand-owns-domain의 우선순위와 safe/danger 충돌 규칙 유지 |
...
| FR-035 | 페이지 추출(`content_extract.js`)은 보안적으로 의미 있는 입력/제출 표면과 일반 UI 버튼을 구분하고, 컨트롤에 accessible label 을 포함해야 한다. | FORMS = form/input/textarea/select + credential·PII·submit 버튼(라벨이 `CREDENTIAL_CTRL_RE` 매칭). UI_CONTROLS = 라벨 있는 일반 버튼(네비/팔로우/카테고리 등) 최대 10, 저신호 슬라이스(priority 4.5, 토큰 압박 시 조기 drop). **라벨 없는 버튼 더미(`<button type="button">`)는 FORMS/UI 양쪽에서 제외** — LLM 입력 오염 방지. 각 컨트롤은 `aria-label`→`aria-labelledby`→`<label for>`→감싸는 `<label>`→자기 텍스트(button)→`title`/`placeholder` 순으로 label 추출해 `label="..."` 로 직렬화. prompt 섹션 분리(FORMS/UI_CONTROLS/TEXT)로 LLM 이 일반 UI 버튼을 credential form 으로 오해하지 않게 한다. 근거: 라벨 없는 버튼 더미가 정상 커뮤니티/리워드 페이지를 "버튼 많은 로그인/보상 페이지"로 보이게 해 FP 유발(ogog 회귀). **visibleText 는 `innerText` 우선이되, 그 값이 사실상 비면(<20자) `textContent` 로 폴백한다(script/style/noscript/svg 는 detached clone 에서 제거). `innerText` 는 레이아웃 의존이라 `active:false` 백그라운드 숨김 탭(contextMenu/회귀/폴백 스캔)에서 빈 문자열을 반환 → body 텍스트 전체가 누락돼 footer 의 운영사/저작권(예: "SK플래닛 주식회사", "Copyright SK PLANET")이 LM·O1-infra-corroboration 에 안 보이던 회귀. 이 폴백으로 ogog 는 페이지에 명시된 "SK플래닛" + NS `skplanet.com` 매칭으로 O1-infra-corroboration 가 발동해 별칭 등록 없이 cap 된다.** |
| FR-039 | LM 세션은 스캔마다 새로 만들지 않고 캐시된 베이스 세션을 재사용해야 한다. | `scanUrl` 은 `ensureSession()` 으로 모듈 스코프 베이스 세션(`_session`)을 1회 생성·캐시하고, 스캔마다 `base.clone()` 으로 독립 컨텍스트를 얻어 prompt 후 **clone 만 destroy**(베이스는 유지). 예전엔 스캔마다 `createLanguageModelSession()`+destroy 로 풀 세션을 churn 했는데, on-device 세션 누적 시 Gemini Nano 가 `create()` 에서 hang 하는 회귀("Starting on-device session" 반복, 추론 미완)가 있었다. `prompt()` 는 45s `Promise.race` 타임아웃으로 감싸고, 오류/타임아웃 시 `_session`/`_sessionPromise` 캐시를 비우고 베이스를 destroy 해 다음 스캔이 새 세션을 만들게 한다(self-heal). `clone` 미지원 환경은 베이스 직접 사용으로 폴백. R6(SW idle 종료) 의 모듈 스코프 캐시 + lazy 재생성 설계와 정합. |
...
| FR-040 | `OFFICIAL_DOMAINS` 무한 확장 대신, 실재 법인 등록 + 도메인 나이라는 증거로 .kr 정상 사이트 FP를 cap 한다. | `O13-established-kr-registrant`: finalHost 가 `.kr` 이고, WHOIS registrant(yesnic `등록인`/`Registrant`, FR-012 로 추출)가 실재 조직(비공백 + privacy/proxy/비공개/정보보호 패턴 아님)이며, `Registered` 등록일 기준 도메인 나이 ≥ 365일이고, **다른 override(danger/warn/safe)가 하나도 없을 때만** score ≤ 3 cap. 특히 O1 brand-mismatch warn(사칭 의심)을 덮지 않게 danger·warn 존재 시 발화 금지. free/shared hosting 제외. brand 이름에 의존하지 않아 LM brand 변동과 무관하고, OFFICIAL_DOMAINS 를 고위험 사칭 표적 seed 로 유지하게 한다. `DOMAIN_TRUST_RULES` 제외(매 스캔 재평가). hard evidence 우선. 예: `howcare.co.kr`(registrant `Gc Healthcare Co.,Ltd`, 2021 등록) → LM score 4 를 3 으로 cap. **한계**: 노후 .kr 을 실명 등록한 soft-lure 피싱(hard evidence 없음)은 cap 될 수 있음 — danger/warn 부재 조건과 신생 도메인 필터로 완화. |
| FR-041 | TLD 무관하게, LM 이 식별한 brand 가 방문 호스트의 등록 도메인과 정확히 일치하면(= 타 브랜드 사칭이 아님) 노후·무-override 조건에서 FP 를 cap 한다. | `O14-brand-owns-domain`: `brandMatchesRegisteredDomain(verdict.brand, finalUrl)` 가 true(brand 정규화 후보 == `registeredDomainLabel`, **정확 일치** — combosquat `lpoint-verify.com` 은 불일치), 도메인 나이 ≥ 365일, free/shared hosting 아님, **다른 override(danger/warn/safe) 전무** 일 때만 score ≤ 3 cap. O13 이 registrant 부재(.com RDAP 는 registrar 만)로 못 잡는 케이스를 brand-domain 동일성으로 보강. 사칭 본질(다른 브랜드 도메인 위장)과 반대 신호라 안전하고, 정확 일치 + 나이로 combosquat/신생 brandable 도메인 탈취를 거른다. O1 brand-mismatch warn 을 덮지 않음(danger/warn 부재 조건). `DOMAIN_TRUST_RULES` 제외. 예: `m.lpoint.com`(brand `Lpoint` == `lpoint.com`, 2008 등록) → LM score 7(callback 파라미터·m 서브도메인 같은 약한 사유) 를 3 으로 cap. |
...
| O13-established-kr-registrant | safe | `.kr` + 실재 법인 registrant(비공백·비-privacy) + 도메인 나이 ≥ 365일 | score <= 3, **다른 override(danger/warn/safe)가 전혀 없을 때만**. 사칭 warn 안 덮음. curation-free FP cap(FR-040). DOMAIN_TRUST_RULES 제외 |
| O14-brand-owns-domain | safe | brand 가 등록 도메인 레이블과 **정확 일치**(사칭 아님) + 도메인 나이 ≥ 365일 + free-hosting 아님 (TLD 무관) | score <= 3, **다른 override(danger/warn/safe) 전무 시만**. combosquat·신생 도메인 거름. curation-free FP cap(FR-041). DOMAIN_TRUST_RULES 제외 |
...
| `chrome.storage.session` | `clickGuardWarnApprovals` | click guard warn 확인을 누른 **host**, 6시간 TTL (entry 는 host + 참고용 urlHash 보유, 조회/dedup 은 host 기준). 재확인 생략 전용이며 스캔/allowlist 우회 아님 |
